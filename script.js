        ...
        class SkinPackMerger {
            ...
            processSkinPackFolders(files) {
                ...
                Object.entries(folderGroups).forEach(([folderName, folderContent]) => {
                    if (folderContent.skins.length > 0) {
                        ...
                        console.warn(`警告: ${folderName} 中没有 skins.json 文件`);
                    }
                });
            }
            ...
            async loadSkinPackFolder(folderName, folderContent) {
                try {
                    ...
                    console.log(`成功加载: ${folderName}`);
                    this.showToast(`已加载 ${folderName}`, 'success');
                    
                } catch (error) {
                    console.error(`加载失败: ${folderName} - ${error.message}`);
            ...
            async readJsonFile(file) {
                ...
                try {
                    ...
                    console.warn(`清理 JSON 注释: ${file.name}`);
            ...
            async processGeometryFiles(geometryFiles) {
                ...
                for (const file of geometryFiles) {
                    try {
                        ...
                        console.warn(`几何模型读取失败: ${file.name}`);
                    }
                }
                ...
                fileCard.innerHTML = `
                    ...
                    <div style="margin-top: 0.75rem; padding-top: 0.75rem; border-top: 1px solid var(--border);">
                        <div style="font-size: 0.75rem; color: var(--text-secondary);">
                            <strong>包名:</strong> ${skinPack.info.serializeName}
                        </div>
                    </div>
                `;
            ...
            async mergeSkinPacks() {
                ...
                try {
                    ...
                    console.error(`合并错误: ${error.message}`);
            ...
            async downloadAsZip() {
                ...
                if (this.mergedResult.others.size > 0) {
                    ...
                    for (let i = 0; i < otherFiles.length; i++) {
                        ...
                        try {
                            ...
                            console.warn(`其他文件处理失败: ${fileName}`);
                        }
                    }
                }
                ...
                console.log(`ZIP 下载完成，大小: ${(zipBlob.size / (1024 * 1024)).toFixed(2)} MB`);
            }
